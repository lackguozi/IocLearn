...
namespace LuckFramework.redis
{
    public class RedisCacheManeger : IRedisCacheManager
    {
       ...
       //private readonly ILogger<RedisBasketRepository> _logger;
        ...
        public RedisCacheManeger(ConnectionMultiplexer redis )
        {
            this.redis = redis;
        ...
        public async Task Set(string key, object value, TimeSpan cacheTime)
        {
            if (value != null)
            {
                ...
                else
                {
                    ...
                    await database.StringSetAsync(key, SerializeHelper.Serialize(value), cacheTime);
                }
            }
        }
        ...
        public async Task<long> ListRightPushAsync(string redisKey, IEnumerable<string> redisValue, int db = -1)
        {
            ...
            return await database.ListRightPushAsync(redisKey, redislist.ToArray());
        }
        ...
        {
            return JsonConvert.DeserializeObject<T>(await database.ListLeftPopAsync(redisKey));
        }
        ...
        {
            return JsonConvert.DeserializeObject<T>(await database.ListRightPopAsync(redisKey));
        }
        ...
        {
            var result = await database.ListRangeAsync(redisKey);
        ...
        {
            var result = await database.ListRangeAsync(redisKey, start, stop);
            return result.Select(o => o.ToString());
        }
        ...
        {
            return await database.ListRemoveAsync(redisKey, redisValue, type);
        }
